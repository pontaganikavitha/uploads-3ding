...
//     const fetchFiles = async () => {
//         try {
//             const response = await fetch(`http://54.206.1.177:3001/files/${orderId}`);
//             const data = await response.json();
//             setFiles(data);
//         } catch (error) {
...
//         if (session && orderId) {
//             setUploaderVisible(true);
//             fetchFiles(); // Fetch files for the entered orderId
//         }
//     }, [session, orderId]);
...
const UploadingFilesWithOrderid = () => {
    const [session, setSession] = useState("");
    const [orderId, setOrderId] = useState("");
    const [files, setFiles] = useState([]);
    const [uploaderVisible, setUploaderVisible] = useState(false);
...
export default UploadingFilesWithOrderid;
