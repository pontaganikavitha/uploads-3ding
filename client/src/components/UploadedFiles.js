...
const UploadedFiles = () => {
  ...
  useEffect(() => {
    const fetchOptionsData = async () => {
      try {
        ...
        const response = await fetch('http://ec2-3-25-81-179.ap-southeast-2.compute.amazonaws.com:3001/options'); // Update this line
        const data = await response.json();
        setOptionsData(data);
      } catch (error) {
        console.error('Error fetching options data:', error);
  ...
  const fetchFiles = async () => {
    try {
      ...
      const response = await fetch(`http://ec2-3-25-81-179.ap-southeast-2.compute.amazonaws.com:3001/files/${session}`); // Update this line
      const data = await response.json();
      setFiles(data);
    } catch (error) {
      console.error('Error fetching session files:', error);
  ...
  const handleSubmitOrder = async () => {
    try {
      ...
      // Change localhost to your live backend URL
      const response = await fetch('http://ec2-3-25-81-179.ap-southeast-2.compute.amazonaws.com:3001/submit-order', { // Update this line
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
