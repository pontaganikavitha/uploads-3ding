...
const AddingNewFileToOrderId = () => {
    ...
    useEffect(() => {
        const fetchOptionsData = async () => {
            try {
                const response = await fetch('http://13.55.42.43:3001/options');
    ...
    const fetchOrderDetails = async (orderId) => {
        try {
            const response = await fetch(`http://13.55.42.43:3001/orders/${orderId}`);
    ...
    const handleSubmitOrder = async () => {
        try {
            ...
            const response = await fetch('http://13.55.42.43:3001/submit-order', {
                method: 'POST',
