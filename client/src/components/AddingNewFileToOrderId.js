...
const AddingNewFileToOrderId = () => {
    ...
    useEffect(() => {
        const fetchOptionsData = async () => {
            try {
                const response = await fetch('http://3.26.98.75/api/options');
    ...
    const fetchOrderDetails = async (orderId) => {
        try {
            const response = await fetch(`http://3.26.98.75/api/orders/${orderId}`);
    ...
    const handleSubmitOrder = async () => {
        try {
            ...
            const response = await fetch('http://3.26.98.75/api/submit-order', {
                method: 'POST',
