...
function getChanges(oldCode, newCode) {
  var changes = diff.diffLines(normalize(oldCode), normalize(newCode));
  var index = 0;
  var chunks = [];
...
function getCodeList(inputSteps) {
  ...
  return inputSteps.map(function (_ref3) {
    var code = _ref3.code,
        lang = _ref3.lang;
    var stepCode = lang === "diff" ? diff.applyPatch(prevCode, code) : code;
    prevCode = stepCode;
    return stepCode;
  });
}
...
//# sourceMappingURL=step-parser.cjs.development.js.map
